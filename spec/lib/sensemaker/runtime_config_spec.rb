...
describe Sensemaker::RuntimeConfig do
  ...
  describe "#backend" do
    it "maps vertex provider to vertex backend" do
      allow(setting).to receive(:[]).with("llm.provider").and_return("VertexAI")
      expect(runtime_config.backend).to eq("vertex")
    end

    it "maps openai provider to openai-compatible backend" do
      allow(setting).to receive(:[]).with("llm.provider").and_return("OpenAI")
      expect(runtime_config.backend).to eq("openai-compatible")
    end

    it "maps ollama provider to ollama backend" do
      allow(setting).to receive(:[]).with("llm.provider").and_return("ollama")
      expect(runtime_config.backend).to eq("ollama")
    end

    it "returns nil for unsupported provider" do
      allow(setting).to receive(:[]).with("llm.provider").and_return("unsupported")
      expect(runtime_config.backend).to be(nil)
