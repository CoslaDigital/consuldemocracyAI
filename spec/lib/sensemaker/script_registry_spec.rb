...
describe Sensemaker::ScriptRegistry do
  describe ".all" do
    it "returns all five Node script ids" do
      expect(Sensemaker::ScriptRegistry.all).to contain_exactly(
  ...
  describe ".for_backend" do
    it "returns all scripts for :node" do
      expect(Sensemaker::ScriptRegistry.for_backend(:node)).to match_array(Sensemaker::ScriptRegistry.all)
    end

    it "returns empty array for unregistered backend" do
      expect(Sensemaker::ScriptRegistry.for_backend(:python)).to eq([])
    end
  end
  ...
  describe ".publishable" do
    it "returns publishable script ids" do
      expect(Sensemaker::ScriptRegistry.publishable).to contain_exactly("runner.ts", "sensemaking-report-ui")
    end
  end
  ...
  describe ".artefact_config" do
    ...
    it "returns config hash for known scripts" do
      ...
      expect(config[:output_basename].call(job)).to eq("categorization-output-42.csv")
    end
  ...
  describe ".scripts_for_logical_name" do
    it "returns all script ids for a logical name" do
      expect(Sensemaker::ScriptRegistry.scripts_for_logical_name(:report)).to eq(["sensemaking-report-ui"])
      expect(Sensemaker::ScriptRegistry.scripts_for_logical_name(:summary)).to eq(["runner.ts"])
    end
  end
